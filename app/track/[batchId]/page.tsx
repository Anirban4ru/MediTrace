    ...
    return (
      <div className="min-h-screen bg-[#F4F4F6] flex items-center justify-center p-4">
        <div className="brutal-card p-8 text-center max-w-md w-full">
          <ShieldAlert className="h-12 w-12 mx-auto mb-4 text-[#B91C1C]" strokeWidth={2} />
          <h1 className="text-[24px] display-heavy uppercase mb-2">Not Found</h1>
          <p className="text-[14px] text-black/70 mono-data">The requested batch could not be found on the ledger.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8">
        ...
        {/* Status Banner */}
        <div className={cn(
          "border-4 border-black p-6 md:p-8 relative overflow-hidden brutal-shadow",
          batch.currentStatus === 'Verified' ? "bg-[#0f5132]/10" :
          batch.currentStatus === 'Spoiled' ? "bg-[#B91C1C]/10" : "bg-[#F4F4F6]"
        )}>
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="mono-data text-[12px] text-black/50 uppercase tracking-[0.1em] mb-1">Product Origin</div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="mono-data text-[12px] bg-black text-white px-2 py-0.5">{batch.manufacturerLabel}</span>
              </div>
            </div>
            
            <div className="bg-white border-2 border-black p-4 text-left md:text-right min-w-[200px]">
              <div className="mono-data text-[10px] text-black/50 uppercase mb-1">Batch Identifier</div>
              <div className="font-bold text-[14px] mono-data mb-2">{batch.batchId}</div>
              <div className="mono-data text-[10px] text-black/50 uppercase mb-1">GS1 Serial</div>
        ...
        {batch.telemetry && batch.telemetry.length > 0 && (
          <div className="brutal-card p-6 border-l-[8px] border-l-[#102A43]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="border border-black/20 p-3 bg-[#F4F4F6]">
                <div className="text-[10px] mono-data text-black/50 uppercase mb-1">Total Pings</div>
                <div className="text-[18px] font-bold mono-data">{batch.telemetry.length}</div>
              </div>
              <div className="border border-black/20 p-3 bg-[#F4F4F6]">
                <div className="text-[10px] mono-data text-black/50 uppercase mb-1">Breaches</div>
                <div className="text-[18px] font-bold mono-data text-[#B91C1C]">
                  {batch.telemetry.filter(t => t.breached).length}
                </div>
              </div>
              <div className="border border-black/20 p-3 bg-[#F4F4F6] col-span-2">
                <div className="text-[10px] mono-data text-black/50 uppercase mb-1">Latest Temp</div>
